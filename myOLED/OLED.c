...
//				if(size2 == 8) OLED_ShowChar(x+(size2/2+2)*t,y,' ',size2);//此句为修改句,消除变量长度大于1重叠问题					
//				else if(size2 == 16) OLED_ShowChar(x+(size2/2)*t,y,' ',size2);//此句为修改句,消除变量长度大于1重叠问题 		
//				continue;
//			}else enshow=1; 
//		 	 
//		}
//	// 	OLED_ShowChar(x+(size2/2)*t,y,temp+'0',size2); //此句为原句，多位数据有重叠(size2 = 8 时，size/2 = 4,字符宽度为6像素，会重叠)
//			if(size2 == 8) 	OLED_ShowChar(x+(size2/2+2)*t,y,temp+'0',size2);//此句为修改句,消除变量长度大于1重叠问题		 			
//			else if(size2 == 16) 	OLED_ShowChar(x+(size2/2)*t,y,temp+'0',size2);//此句为修改句,消除变量长度大于1重叠问题		
//		
//	}
//} 
//显示一个字符串
...
{//参数：图片起始列坐标， 行坐标（页），结束列坐标，结束行坐标，  图片名，  移动方向， 移动速度
	static unsigned char bmx=0;
	if(BMPfx == LEFT)		//改版x坐标要在64~128间（有bug）//只能实现从最右到最左，图片只能突然出现及突然消失然后重复（不能实现从OLED一点点出现的效果）																					//方向：左
	{						//勉强做到敌人从右侧出现的结果
		if(bmx >= 128)bmx = 0;
		if(bmx <= 128)																												
		{	
			if(BMPx1-bmx>0){
				OLED_DrawBMP_Faster(BMPx1-bmx,BMPy1,BMPx2-bmx,BMPy2,BMPn);								//按当前坐标更新图形
				OLED_DrawBMP_Faster(BMPx2-bmx,BMPy1,BMPx2+8-bmx,BMPy2,BMPclr);			//擦除右侧残影
				bmx += 8;			//左移8列像素
			}
			else{
				OLED_DrawBMP_Faster(0,BMPy1,BMPx2-BMPx1,BMPy2,BMPclr);//擦除残影
				bmx=0;
			}
		}
	...
	if(BMPfx == RIGHT)		//改版x坐标要在0~64间（有bug）（一样）																								//方向：右
	{
		if(bmx >= 128) bmx = 0;
		if(bmx <= 128)																											
		{
			if(bmx+BMPx2<144){
				OLED_DrawBMP_Faster(bmx+BMPx1,BMPy1,bmx+BMPx2,BMPy2,BMPn);								//按当前坐标更新图形
//				OLED_DrawBMP_Faster(bmx+BMPx2,BMPy1,bmx+BMPx2+8,BMPy2,BMPclr);	
				OLED_DrawBMP_Faster(bmx+BMPx1-8,BMPy1,bmx+BMPx1,BMPy2,BMPclr);//擦除左侧残影
				bmx +=8;				//右移8列像素
				}
			else{
				OLED_DrawBMP_Faster(128-(BMPx2-BMPx1),BMPy1,128,BMPy2,BMPclr);//擦除残影
				bmx=0;
			}
		}
	}	
